...
class IntegrationLoss:
    ...
    def losscontactEnergy(self, f, x=None):
        if self.type == 'lineIntegral':
            # print("Trapezoidal rule")
            if self.dim == 2:
                if x is not None:
                    return self.trapzContact1D(f,x)
                else:
                    return self.trapz1D(f, x)



    def trapzContact1D(self,f,P):
        I_sum = torch.Tensor([0])
        for i in range(P.shape[0]-1):
            r = lambda t: torch.Tensor([(1-t) * P[i,:] + t*P[i+1,:]])
            rd = lambda t:  -1 * P[i,:] + P[i+1,:]

            I_sum += (1-0)*(0.5*( (f[i]* torch.norm(rd(0)))   + (f[i+1]* torch.norm(rd(1)))))
            if (1-0)*(0.5*( (f[i]* torch.norm(rd(0)))   + (f[i+1]* torch.norm(rd(1))))) == 2:
                print('here')
        ...
        if self.type == 'trapezoidal':
            ...
            if self.dim == 2:
                if x is not None:
                    ...
                    return self.trapz2D_Batch(f, dx=dx, dy=dy, shape=shape) #self.trapz2D(f, dx=dx, dy=dy, shape=shape)
            if self.dim == 3:
                if x is not None:
    ...
    def trapz2D_Batch(self, f, xy=None, dx=None, dy=None, shape=None):
        #f2D = f.reshape(shape[0], shape[1])
        if dx is None and dy is None:
            x = xy[:, 0].flatten().reshape(shape[0], shape[1])
            y = xy[:, 1].flatten().reshape(shape[0], shape[1])
            ...
            intVal = torch.tensor(0.0).float()
            for i in range(f.shape[0]):
                intVal = intVal + self.trapz(self.trapz(f[i,0,:,:], dx=dy), dx=dx)
            return intVal


    def trapz2D_HS(self, f, r, theta, shape):
        n = len(r)
        m = len(theta)
        f2D = f.reshape(shape)
        sum_T = torch.Tensor([0])
        for i in range(n-1):
            for j in range(m-1):
                I1 = (r[i + 1] - r[i]) * 0.5 * (f2D[i,j] * r[i] + f2D[i+1,j] * r[i + 1])
                I2 = (r[i + 1] - r[i]) * 0.5 * (f2D[i,j+1] * r[i] + f2D[i+1,j+1] * r[i + 1])
                sum_T = sum_T + (theta[j + 1] - theta[j]) * 0.5 * (I1 + I2)
